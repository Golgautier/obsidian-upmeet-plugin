...
// languages/fr.json
var fr_default = {
  panelDetailsTitle: "D\xE9tails du meeting",
  panelNoNoteSelected: "Pas de note s\xE9lectionn\xE9e",
  panelNotAUpmeetNote: "Ne semble pas \xEAtre une note Upmeet",
  panelDetailMeetingID: "ID R\xE9union : ",
  panelDetailAuthor: "Auteur : ",
  panelDetailCreated: "Cr\xE9\xE9e le : ",
  panelDetailProcessed: "Trait\xE9e le : ",
  panelDetailTags: "Tags : ",
  panelButtonSync: "Sync les r\xE9unions",
  upmeetToken: "Token Upmeet",
  upmeetTokenDesc: "Le token Upmeet est utilis\xE9 pour authentifier les requ\xEAtes vers l'API Upmeet. Il peut \xEAtre trouv\xE9 dans les param\xE8tres de votre compte Upmeet.",
  upmeetTokenPlaceholder: "Entrez votre token Upmeet ici",
  upmeetFolder: "Dossier des notes",
  upmeetFolderDesc: "Le dossier o\xF9 les notes Upmeet seront stock\xE9es. Si le dossier n'existe pas, il sera cr\xE9\xE9.",
  upmeetFolderPlaceholder: "Entrez le chemin du dossier pour les notes Upmeet",
  upmeetLastSyncDate: "Date de derni\xE8re synchronisation",
  ...
  noticeSyncStarting: "Synchronisation avec Upmeet en cours...",
  noticeSyncCompleted: "Synchronisation avec Upmeet termin\xE9e.",
  noticeFileAltreadyExistsSkip: "Le fichier existe d\xE9j\xE0 et va \xEAtre pass\xE9 : ",
  noticeUpmeetConnectionError: "\xC9chec de la r\xE9cup\xE9ration des r\xE9unions depuis l'API Upmeet. Veuillez consulter la console pour plus de d\xE9tails.",
  noticeNewMeetingsFound: "Nouvelles r\xE9unions trouv\xE9es : ",
  defaultNoteContent: "Tags: {{tags}}\n\n# R\xE9sum\xE9:\n\n{{summary}}\n\n---\n# Transcription:\n\n{{transcription}}"
};
...
// upmeet/upmeet.ts
async function getMeetingSummary(meetingId, upmeetToken) {
  const url2 = `https://api.upmeet.ai/meetings/${meetingId}`;
  const headers = {
    "Accept": "application/json",
    "X-Api-Token": upmeetToken
  };
  try {
    const response = await axios_default.get(url2, { headers });
...
async function getMeetingTranscription(meetingId, upmeetToken) {
  const url2 = `https://api.upmeet.ai/meetings/${meetingId}/transcriptions`;
  const headers = {
    "Accept": "application/json",
    "X-Api-Token": upmeetToken
  };
  try {
    ...
    return transcriptionData.transcriptions.map((tmp) => {
...
async function getMeetings(plugin) {
  let returnedMeetings = [];
  let currentPage = 1;
  let totalPages = 1;
  let lastMeeting = "";
  let response;
  const url2 = "https://api.upmeet.ai/meetings";
  const headers = {
    "Accept": "application/json",
  ...
  while (currentPage <= totalPages) {
    try {
      response = await axios_default.get(`${url2}?page=${currentPage}`, { headers });
    } catch (error) {
      console.error(`Error fetching meetings from Upmeet API: ${error}`);
      new import_obsidian.Notice(t(plugin.language, "noticeUpmeetConnectionError"));
...
// main/mainfunctions.ts
function updatePanel(plugin, file) {
...
function toggleRightPanel(plugin) {
  const leaves = plugin.app.workspace.getLeavesOfType(PANEL_NAME);
  if (leaves.length > 0) {
...
async function syncNotes(plugin) {
  new import_obsidian2.Notice(t(plugin.language, "noticeSyncStarting"));
  let lastMeetingDate;
  let meetingsList = [];
  let templateContent = "";
  if (plugin.settings.upmeetTemplate) {
    ...
    templateContent = t(plugin.language, "defaultNoteContent");
  }
  ...
  new import_obsidian2.Notice(t(plugin.language, "noticeNewMeetingsFound") + `${meetingsList.length}`);
...
`;
  ...
  new import_obsidian2.Notice(t(plugin.language, "noticeSyncCompleted"));
}
async function createNewNote(plugin, title, content) {
  ...
  if (existingFile) {
    if (plugin.settings.overwriteExistingNotes) {
      ...
      new import_obsidian2.Notice(t(plugin.language, "noticeFileAltreadyExistsSkip") + filename);
      return;
    }
  }
...
// panel/panelfunctions.ts
function updateDetailsfromPanel(item, file) {
...
function updatePanel2(item, file) {
...
function createPanel(item) {
  ...
  (0, import_obsidian3.setIcon)(btn, "refresh-cw");
  btn.onclick = () => {
...
// panel/panel.ts
var PANEL_NAME = "upmeetPanel";
var upmeetPanelView2 = class extends import_obsidian4.ItemView {
  // Default language, can be set from the plugin
  ...
  refreshPanel(file) {
    updatePanel2(this, file);
  }
  // This method is called when the panel is opened
  async onOpen() {
    this.language = this.plugin.language;
    createPanel(this);
  }
  async onClose() {
...
var DEFAULT_SETTINGS = {
  upmeetToken: "default",
  upmeetFolder: "Upmeet",
  upmeetLastSync: "2000-01-01T00:00:00Z",
  // Default date in the past
  upmeetTemplate: "",
  // Template for new notes
  overwriteExistingNotes: false
  // Default to not overwriting existing notes
};
var GolUpmeetPlugin = class extends import_obsidian5.Plugin {
  ...
  async onload() {
    ...
    const obsidianLanguage = import_moment.default.locale();
    this.language = obsidianLanguage === "fr" ? "fr" : "en";
    if (!this.app.workspace.getLeavesOfType(PANEL_NAME).length) {
      console.debug(`Initializing Upmeet panel view on load`);
      this.registerView(PANEL_NAME, (leaf) => new upmeetPanelView2(leaf, this));
    }
    if (this) {
      this.registerEvent(
        this.app.workspace.on(
          "active-leaf-change",
          (0, import_obsidian5.debounce)((leaf) => {
    ...
    (0, import_obsidian5.addIcon)(CUSTOM_ICON_ID, CUSTOM_ICON_SVG);
    this.addRibbonIcon(CUSTOM_ICON_ID, "Upmeet", () => {
...
var SampleSettingTab = class extends import_obsidian5.PluginSettingTab {
  ...
  display() {
    ...
    containerEl.empty();
    new import_obsidian5.Setting(containerEl).setName(t(this.plugin.language, "upmeetToken")).setDesc(t(this.plugin.language, "upmeetTokenDesc")).addText((text) => {
      text.setPlaceholder(t(this.plugin.language, "upmeetTokenPlaceholder")).setValue(this.plugin.settings.upmeetToken).onChange(async (value) => {
        this.plugin.settings.upmeetToken = value;
        await this.plugin.saveSettings();
      });
      text.inputEl.setAttribute("type", "password");
    });
    new import_obsidian5.Setting(containerEl).setName(t(this.plugin.language, "upmeetFolder")).setDesc(t(this.plugin.language, "upmeetFolderDesc")).addText((text) => text.setPlaceholder(t(this.plugin.language, "upmeetFolderPlaceholder")).setValue(this.plugin.settings.upmeetFolder).onChange(async (value) => {
      this.plugin.settings.upmeetFolder = value;
      await this.plugin.saveSettings();
    }));
    new import_obsidian5.Setting(containerEl).setName(t(this.plugin.language, "upmeetLastSyncDate")).setDesc(t(this.plugin.language, "upmeetLastSyncDateDesc")).addText((text) => {
      text.setPlaceholder(t(this.plugin.language, "upmeetLastSyncDatePlaceholder")).setValue(this.plugin.settings.upmeetLastSync).onChange(async (value) => {
        this.plugin.settings.upmeetLastSync = value;
        await this.plugin.saveSettings();
      });
    });
    new import_obsidian5.Setting(containerEl).setName(t(this.plugin.language, "upmeetTemplate")).setDesc(t(this.plugin.language, "upmeetTemplateDesc")).addTextArea((text) => {
      text.setPlaceholder(t(this.plugin.language, "upmeetTemplatePlaceholder")).setValue(this.plugin.settings.upmeetTemplate).onChange(async (value) => {
        this.plugin.settings.upmeetTemplate = value;
        await this.plugin.saveSettings();
      });
    });
    new import_obsidian5.Setting(containerEl).setName(t(this.plugin.language, "upmeetOverwrite")).setDesc(t(this.plugin.language, "upmeetOverwriteDesc")).addToggle(
      (toggle) => {
        toggle.setValue(this.plugin.settings.overwriteExistingNotes).onChange(async (value) => {
